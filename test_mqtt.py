...
Krav: paho-mqtt
Använd:
  python3 test_mqtt.py --host <HOST> --user <USER> \
      --password-env MQTT_PASSWORD --topic genio/pi5/req --payload "hej"
"""
import argparse
import os
import time
...
def main():
    ...
    ap.add_argument("--subscribe", default=None,
                    help="Valfritt: topic att lyssna på efter publish")
    args = ap.parse_args()

    pw = os.environ.get(args.password_env)
    if not pw:
        raise SystemExit(
            f"Env-var {args.password_env} saknas "
            f"(exportera lösenordet innan körning)."
        )
