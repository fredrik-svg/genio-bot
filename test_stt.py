...
Använd:
  python3 test_stt.py --wav /path/to/file.wav --model tiny --lang sv
eller spela in nytt klipp (5s):
  python3 test_stt.py --record-seconds 5 --out /tmp/utt.wav --model small --lang sv
"""
import argparse
import wave
import sys
...
def record_wav(seconds: int, out_path: str, device=None):
    ...
    data = sd.rec(int(seconds*SR), samplerate=SR, channels=1, dtype="int16", device=device)
    sd.wait()
...
def main():
    ...
    ap.add_argument("--record-seconds", type=int, help="Spela in så här många sekunder")
    ap.add_argument("--out", default="/tmp/utt.wav", help="Väg för inspelad WAV (om --record-seconds används)")
    ap.add_argument("--model", default="tiny", help="Whisper-modell (tiny, base, small, ...)")
    ap.add_argument("--lang", default="sv", help="Språk, t.ex. sv eller en")
